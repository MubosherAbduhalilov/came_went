		...
		if len(qach_e.get().split())==3 and bir(qach_e.get().split()):
			...
			if len(f3)!=0:
				for i in range(len(f3)):
					...
					qach_e.delete(0,END)
					ism=Label(top,text=f'Ism-familiya - {f3[1]}',font=("consolass",18,"bold"),bg=top['bg'])
					ism.grid(row=5, columnspan=2,sticky="W",padx=(15,15))
					telraq = Label(top, text=f'Telefon raqam - {f3[2]}', font=("consolass", 18, "bold"),bg=top['bg'])
					...
					kel = Label(top, text=f'Keldi - {f3[4]}', font=("consolass", 18, "bold"),bg=top['bg'])
					kel.grid(row=8, columnspan=2,sticky="W",padx=(15,15))
					ket = Label(top, text=f'Ketdi - {f3[5]}', font=("consolass", 18, "bold"),bg=top['bg'])
...
def ish_mal(bkv,bktv):
		...
		def keldi_ketdilar(count):
			...
			if count<len(s):
				try:
					...
					if count==len(ls):
						xatolik1 = Label(top,text="Bugun hamma ishchilarning keldi-ketdisi to\'ldirib bo\'lingan",font=("consolass", 13, "bold"), bg=top["bg"], fg="red")
						xatolik1.grid(row=1, column=0, pady=(10, 10),padx=(10,0))
					else:
						top.destroy()
			elif len(s)==0:
				xatolik2 = Label(top, text="Afsuski, bazada ishchilar yo\'q,\nasosiy oynaga o\'tib ishchilar qo\'shing", font=("consolass", 16, "bold"), bg=top["bg"],fg="red")
	...
	def oylikchiq():
		...
		if len(cur.fetchall())>0:
			...
			xatolik3.grid(row=1, columnspan=2, pady=(3, 3))
			kimdir = Label(top, text="Ishchini kiriting", font=("consolass", 16, "bold"), bg=root['bg'])
			kimdir.grid(row=2, column=0, pady=(5, 5))
			kimdir_e = Entry(top, font=("consolass", 18, "bold"), width=20)
			kimdir_e.grid(row=2, column=1,sticky="W")
			ogoh2 = Label(top, text='Ishchi nomiga yaxshilab e\'tibor bering', font=("consolass", 13, "bold"),bg=root['bg'], fg='red')
			ogoh2.grid(row=3, columnspan=2)
